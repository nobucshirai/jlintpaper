...
$regexp = [
  ...
  # ほか（特別の場合を除くほか）
  # 外→ほか
  {'regexp'=>/(全て|の時[^点間計刻]|の通り|有る|在る|居る|成る|出来|て上げる|て行く|て置く|て来る|て仕舞う|て見る|無い|\p{^Han}様\p{Hiragana}|\p{^Han}位{^Han}|丈|[^過]程[^度]|以って|且つ|但し|但(?=書)|従って|因る|(?<!その)他[^の\p{Han}\p{Katakana}]|(?<![の\p{Han}])外[^\p{Han}れ])/,
